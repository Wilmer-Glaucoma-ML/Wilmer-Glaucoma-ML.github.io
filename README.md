There are two branches to this repo: `source` and `master`. Update `source` with the changes and then run `bin/deploy --user` to update master and automatically rebuild the website with the new changes.
